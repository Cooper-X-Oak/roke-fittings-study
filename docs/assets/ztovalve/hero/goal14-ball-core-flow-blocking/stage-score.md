...
- Viewer arc: 真实不锈钢产品 -> 球心锚定 -> 多方向解剖 -> 阀座/密封围绕球体但保持固定 -> 球体单件 90 度启闭 -> 流线通过 -> 商业回正。
- Dominant staged-image rule: 球体是视觉锚点；动作可以大，但所有方向都必须服务球体、轴线和通路。
- Camera grammar: 固定三分之四商业观察角，微推近和微回正；不做自由 orbit，不追随小五金。
- Product grammar: 球体保持中心；左右阀体沿管道轴离散；阀杆/驱动区向上；下支撑向下；阀座/密封环围绕球体左右分层但不随球转动；只有真实球芯单件做 90 度功能旋转。
...
- `ballTurn` 可以明显，但只能作用于真实球芯单件；阀座、阀体、密封圈、支撑件和小五金不得跟转。
